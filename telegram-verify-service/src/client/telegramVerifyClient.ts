...
export class TelegramVerifyClient {
  ...
  private async request<T>(path: string, body: Record<string, unknown>): Promise<T> {
    ...
    try {
      ...
      if (!json) {
        throw new TelegramVerifyError("BAD_RESPONSE", "Invalid response from verify service.", res.status);
      }
      if (!json.ok) {
        throw new TelegramVerifyError(json.error.code, json.error.message, res.status, json.error.retryAfter);
      }
      return json.data;
    } catch (err) {
