  ...
  const { data: courses = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("courses")
        .select("id, title, description, cover_url, category, published, lessons(count)")
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {isLoading ? "Yuklanmoqda..." : `${courses.length} ta kurs mavjud`}
            </p>
          </div>
          <Button asChild>
                ...
                <div className="mt-4 flex gap-2">
                  <Button asChild variant="outline" size="sm" className="flex-1">
                    <Link to="/admin/courses/$courseId" params={{ courseId: c.id }}>
                      <Edit className="mr-1 h-3.5 w-3.5" /> Tahrirlash
                    </Link>
                  </Button>
                  <Button
                    variant="outline"
                    ...
                    onClick={() =>
                      confirm(`"${c.title}" kursini o'chirasizmi?`) && del.mutate(c.id)
                    }
                  >
